    ...
    class Program
    {
        ...
        public bool runNamd(Program ob)
        {
            if (ob.executesubjob(jobId))
            {
                ...
                Trace.WriteLine(" ########## Namd Ran for First Time ########### added message to queue.........Now UpdatedState : Done " + jobId);
        ...
        public void callReplicaExchange(Program ob)
        {
            ...
            CloudQueueMessage queueMessage1;
            Trace.WriteLine("############## Job ID Arguments in Replica-Exchange ################# :" + jobId);
            Dictionary<string, object> jobDict_current = getJobDictFromBlob(jobId);
            Dictionary<string, object> jobDict;
            ...
            while (queue1.PeekMessage() != null)
            {
                ...
                queueMessage1 = queue1.GetMessage();
                ...
                {
                       Trace.WriteLine("In CheckDone State method", "Information");
                       UpdateState(jobId, "Free");
                ...
                if (jobId != queueMessage1.AsString)
                {
                    ...
                    jobDict = getJobDictFromBlob(f_jobId);
                    //queue1.DeleteMessage(queueMessage1);
                    if (ob.checkStateFree(ob, queueMessage1.AsString, jobDict))
                    {
                           // Change the state of present jobId and post it to blob 
                            ...
                            UpdateState(queueMessage1.AsString, "Pending");
                        ...
                        ob.temp_b = ob.queueMessageTemperature(ob, queueMessage1.AsString, jobDict);  // doubt
                        ...
                        // Restart both Replicas after Updating the States and increase the count variable in blob 
                            ...
                            UpdateState(queueMessage1.AsString, "Ready");
        ...
        public int queueMessageTemperature(Program ob, String jobId, Dictionary<string, object> jobDict)
        {
            Trace.WriteLine(" Before Temperature of Message Queue Job Id is: " + ob.temp_b);
            if (jobDict.ContainsKey("arguments"))
            {
                string jqueuemessageArray = jobDict["arguments"].ToString();
        ...
        public bool checkStateDone(Program ob, String jobId, Dictionary<string, object> jobDict)
        {
            if ((string)jobDict[STATE] == "Done")
            {
                Trace.WriteLine(" ######### Yes In done state ##########", "Information");
                Trace.WriteLine(" job id is : " + jobId);
                Trace.WriteLine(" ");
                return true;
            }
        ...
        public bool checkStateReady(Program ob, String jobId, Dictionary<string, object> jobDict)
        {
            if ((string)jobDict[STATE] == "Ready")
            {
                Trace.WriteLine(" ######### In CheckReady State method ##########", "Information");
                Trace.WriteLine(" $$$$$$$$$ Will Restart the current JobID soon $$$$$$$$$$$$ ", jobId);
                return true;
            }
            else
                return false;
        ...
        public bool checkStateFree(Program ob, String jobId, Dictionary<string, object> jobDict)
        {
            lock (lockThis)
            {
                if ((string)jobDict[STATE] == "Free")
                {
        ...
        public int incrementBlobCount()
        {
            ...
            try
            {
                ...
                int blobcount = Convert.ToInt32(countBlob.DownloadText());
                count += 1;
                //Upload to Blob /
                countBlob.UploadText(blobcount.ToString());
                Trace.WriteLine("IncrementBlobCount value is: " + count);
                return count;
            }
            catch (Exception e)
            {
        ...
        public double getEnergy(Program ob, String jobId)
        {
